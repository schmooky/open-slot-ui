...
// @open-slot-ui/pixi is the renderer binding. Keep the heavy, instance-sensitive deps
// OUT of the bundle (Charter B2 — one shared pixi.js): `pixi.js` is a peer and
// `@open-slot-ui/core` ships separately, so both stay external.
//
...
// vendored 0.2.0 that isn't on npm, so inlining keeps `@open-slot-ui/pixi` self-
...
export default defineConfig({
  ...
  // `@open-slot-ui/core` and `pixi.js` stay external in the types (they're real deps).
  dts: { resolve: ['pixi-text-counter'] },
  sourcemap: true,
  clean: true,
  treeshake: true,
  external: ['pixi.js', '@open-slot-ui/core'],
});
