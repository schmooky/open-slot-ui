...
/**
 * Autoplay picker as a BOTTOM DRAWER. On the autoplay control's `'picking'` state
 * (an options-mode tap) it slides up, dims the HUD, and offers the count choices —
 * plus, when configured, the responsible-gambling limits: "stop on loss" and "stop
 * on single win" (multiples of bet; ∞ = no limit). Pick → `autoplay.pick(count,
 * {lossLimit, singleWinLimit})`; the host then enforces the limits by feeding each
 * round to `hud.reportRound(win, bet)`. Tap the backdrop → `cancelPicker()`.
 *
 * A full-screen overlay (managed by OpenUIPixi's overlay list), so it never touches
 * the autoplay button's introspection bounds. Themed entirely from tokens.
 */
export class AutoplayDrawerView extends Container {
  ...
  constructor(private readonly auto: AutoplayControl, private readonly ui: OpenUI, private readonly ticker: Ticker) {
    ...
    // Background + handle + title first so they sit BEHIND the chips.
    this.sheet.addChild(this.sheetBg, this.handle, this.title);

    // Count is always offered; the RG limit groups appear only when configured.
  ...
  private addGroup(kind: GroupKind, values: number[], headingText?: string): void {
    ...
    if (headingText) {
      group.heading = new Text({ text: this.ui.t(headingText), style: { fontFamily: th.type.family, fontSize: 16, fill: th.color.textDim, fontWeight: '700', letterSpacing: 0.5 } });
    ...
    values.forEach((value) => {
      ...
      const label = new Text({ text: chipText(kind, value), style: { fontFamily: th.type.family, fontSize: 24, fill: th.color.text, fontWeight: '800' } });
  ...
  private relayout(): void {
    ...
    let y = 22 * k + 30 * k + 20 * k; // below the title
    ...
    this.sheetBg.clear().roundRect(0, 0, W, this.sheetH + 40, 28 * k).fill({ color: th.color.surface }).stroke({ width: 2, color: th.color.surfaceAlt });
...
function defaultSelected(kind: GroupKind, values: number[]): number {
  ...
  // limits default to "no limit" (∞) when offered
